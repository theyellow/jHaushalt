...
public class DlgBereinigen extends JDialog {
  ...
  public DlgBereinigen(final Haushalt haushalt, final Datenbasis db) {
    ...
    comboBox.addActionListener(new ActionListener() {
      public void actionPerformed(ActionEvent e) {
        if(comboBox.getSelectedIndex() == 0)
          ...
          tableModel.setKategorie((EinzelKategorie) comboBox.getSelectedItem());
      }
    });
    ...
    checkBox.addItemListener(new ItemListener() {
      public void itemStateChanged(ItemEvent e)  {
        kategorienEinlesen();
        tableModel.setUnterkategorien(checkBox.isSelected());
      }
    });
    ...
    buttonZuordnen.addActionListener(new ActionListener() {
      public void actionPerformed(ActionEvent e) {
        ...
        EinzelKategorie neueKategorie = (EinzelKategorie) list.getSelectedValue();
  ...
  private void kategorienEinlesen() {
    ...
    EinzelKategorie[] kategorien = db.getKategorien(checkBox.isSelected());
    comboBox.addItem(res.getString("any_category"));
    for (int i = 0; i < kategorien.length; i++) {
      comboBox.addItem(kategorien[i]);
      listModel.addElement(kategorien[i]);
    }
    if(selectedItem == null)
      comboBox.setSelectedItem(EinzelKategorie.SONSTIGES);
    else
