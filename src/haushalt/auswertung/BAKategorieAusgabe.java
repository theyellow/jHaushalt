...
public class BAKategorieAusgabe extends AbstractBlockAuswertung {
  ...
  public BAKategorieAusgabe(Haushalt haushalt, Datenbasis db, String name) {
  ...
  protected String berechneAuswertung(Object[] werte) {
    ...
    String register = (String) werte[1];
    EinzelKategorie[] kategorien = (EinzelKategorie[]) werte[2];

    boolean unterkategorienVerwenden = ((MehrereKategorienGDP)panes[2]).getUnterkategorienVerwenden();
    ArrayList<String[]> buchungen = db.getBuchungen(zeitraum, register, kategorien, unterkategorienVerwenden);
    int anzahl = buchungen.size();
    tabelle = new String[anzahl][4];
    Euro summe = new Euro();
    for(int i=0; i<anzahl; i++) {
      summe.sum(new Euro(buchungen.get(i)[3]));
      tabelle[i] = buchungen.get(i);
    }
    if(DEBUG)
    ...
    if(anzahl == 0) {
      String hinweis = res.getString("no_bookings_with_this_properties");
			AbstractBlock block2 = new TextBlock(hinweis);
      ...
      TabellenBlock block2 = new TabellenBlock(tabelle);
	    ...
	    TabellenBlock block3 = new TabellenBlock(text);
			block3.setFont(new Font(haushalt.getFontname(), Font.ITALIC, haushalt.getFontgroesse()));
