...
public class BARelativerVergleich extends AbstractBlockAuswertung {
  ...
  public BARelativerVergleich(Haushalt haushalt, Datenbasis db, String name) {
  ...
  protected String berechneAuswertung(Object[] werte) {
    ...
    String register = (String) werte[2];
		double faktor1 = 30.0D / zeitraum1.getAnzahlTage();
		double faktor2 = 30.0D / zeitraum2.getAnzahlTage();
    ...
    tabelle[anzKat+1][0] = res.getString("total");
		...
		for(int i=0;i<anzKat;i++) {
			Euro relWert1 = summen1[i].mal(faktor1);
			Euro relWert2 = summen2[i].mal(faktor2);
			tabelle[i+1][0] = ""+kategorien[i];
			tabelle[i+1][1] = ""+relWert1;
			tabelle[i+1][2] = ""+relWert2;
			tabelle[i+1][3] = ""+relWert1.sub(relWert2);
			summe1.sum(relWert1);
			summe2.sum(relWert2);
		}
		...
		String titel = res.getString("relative_comparison")+" ("+tabelle[0][1]+" "+res.getString("and")+" "+tabelle[0][2];
		if(register == null)
		  titel += ")";
		else
		  titel += ", " + register + ")";
    ...
    addDokumentenBlock(new LeererBlock(1));
    TextBlock block3 = new TextBlock(res.getString("calculated_amounts"));
