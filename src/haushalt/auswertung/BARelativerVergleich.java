...
public class BARelativerVergleich extends AbstractBlockAuswertung {
	...
	public static final String UEBERSCHRIFT = TextResource.get().getString("headline_relative_comparison");
	...
	protected String berechneAuswertung(final Object[] werte) {
		...
		for (int i = 0; i < anzKat; i++) {
			...
			setTabelleContent(i + 1, 1, "" + relWert1);
			setTabelleContent(i + 1, 2, "" + relWert2);
			setTabelleContent(i + 1, 3, "" + relWert1.sub(relWert2));
			summe1.sum(relWert1);
			summe2.sum(relWert2);
		}
		...
		String titel = RES.getString("relative_comparison")
			+ " ("
			+ getTabelle()[0][1]
			+ " "
			+ RES.getString("and")
