...
public class TortenBlock extends AbstractGraphikBlock {
  ...
  public TortenBlock(String farbschema,  Euro[] einnahmen, 
    Euro gesamtEinnahmen, Euro[] ausgaben, Euro gesamtAusgaben) {
  ...
  public int paint(Graphics g, int xStart, int yStart, int breite) {
    ...
    int durchmesser = getHoehe(breite);
    int xEinnahmen = xStart+ rand + (breite - 2*getAbsRand(breite)) / 40;
    int xAusgaben = xStart + rand + (breite - 2*getAbsRand(breite)) * 21 / 40;
    Euro[] werte;
    Euro gesamt;
    int x;
    double winkel;
    ...
    for(int i=0; i<2; i++) {
      ...
      for(int j=0; j<werte.length && werte[j]!=null; j++) {
        g2.setPaint(FarbPaletten.getFarbe(j, farbschema));
